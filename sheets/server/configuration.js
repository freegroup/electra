...
const LOCAL_DATADIR_SHEETS = path.resolve(__dirname+ "/..")

module.exports = {

    absoluteGlobalDataDirectory: () => {
        return path.normalize(`${LOCAL_DATADIR_SHEETS}/data/global/`)
    },

    absoluteUserDataDirectory: ( req ) => {
        let hash = req.get("x-hash")
        return path.normalize(`${LOCAL_DATADIR_SHEETS}/data/user/${hash}/`)
    },
